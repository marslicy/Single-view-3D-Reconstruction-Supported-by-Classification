...
class Model(nn.Module):
    def __init__(self, global_feature_size=128, local_feature_size=128):
        """
        ...
        self.view_enc = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=96, kernel_size=7, stride=1),
            ...
            nn.MaxPool2d(kernel_size=2),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Linear(in_features=268, out_features=global_feature_size),
            nn.ReLU(),
        )
        self.class_enc = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=2),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Linear(in_features=268, out_features=local_feature_size),
            nn.ReLU(),
        )
        ...
        self.shape_dec = nn.Sequential(
            ...
            nn.ConvTranspose3d(
                in_channels=128, out_channels=1, kernel_size=5, stride=2
            ),
        )
    ...
    def forward(self, x_in_class: torch.Tensor, x_in_3d: torch.Tensor):
        ...
        pred_3d = self.shape_dec(torch.cat((class_emb, view_emb), dim=0))
